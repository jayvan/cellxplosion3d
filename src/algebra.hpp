...
public:
  ...
  void floor(const Point3D& other) {
    if (other[0] < v_[0]) v_[0] = other[0];
    if (other[1] < v_[1]) v_[1] = other[1];
    if (other[2] < v_[2]) v_[2] = other[2];
  }

  void ceil(const Point3D& other) {
    if (other[0] > v_[0]) v_[0] = other[0];
    if (other[1] > v_[1]) v_[1] = other[1];
    if (other[2] > v_[2]) v_[2] = other[2];
  }
...
public:
  ...
  double biggestComponent() const {
    double max = v_[0];
    if (v_[1] > max) max = v_[1];
    if (v_[2] > max) max = v_[2];
    return max;
  }
...
{
  return Point3D(a[0]+b, a[1]+b, a[2]+b);
}
...
public:
  ...
  Colour lerp(Colour c, double dist) {
    return Colour((c.r_ - r_) * dist + r_,
                  (c.g_ - g_) * dist + g_,
                  (c.b_ - b_) * dist + b_);
  }
