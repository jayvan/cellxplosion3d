...
int gr_disk_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new Disk());
...
// Create a tetrahedron node
extern "C"
int gr_tetrahedron_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new Tetrahedron());
